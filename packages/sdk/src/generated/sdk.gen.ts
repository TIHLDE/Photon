 ...
 * Upload a file
 *
 * Upload a file to staging. The file will be automatically cleaned up after 2 days if not promoted.
 *
 ...
 */
export const uploadAsset = <ThrowOnError extends boolean = true>(options?: Options<UploadAssetData, ThrowOnError>) => {
    return (options?.client ?? client).post<UploadAssetResponses, UploadAssetErrors, ThrowOnError>({
        url: '/api/assets',
        ...options
    });
};
 ...
 * Get asset metadata
 *
 * Get metadata for an asset by its key. No authentication required.
 *
 * The key is the full path returned when uploading, e.g., `uploads/2024/01/uuid_filename.jpg`
 */
export const getAssetMetadata = <ThrowOnError extends boolean = true>(options: Options<GetAssetMetadataData, ThrowOnError>) => {
    return (options.client ?? client).get<GetAssetMetadataResponses, GetAssetMetadataErrors, ThrowOnError>({
        responseTransformer: getAssetMetadataResponseTransformer,
        url: '/api/assets/metadata/{key}',
        ...options
    });
};
 ...
 * Download a file
 *
 * Download a file by its key. No authentication required.
 *
 * The key is the full path returned when uploading, e.g., `uploads/2024/01/uuid_filename.jpg`
 */
export const downloadAsset = <ThrowOnError extends boolean = true>(options: Options<DownloadAssetData, ThrowOnError>) => {
