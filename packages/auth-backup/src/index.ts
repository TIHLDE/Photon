...
export const createAuth = (
    ...
    betterAuth({
        ...
        plugins: [
            ...
            username(),
            jwt({
                // Recommended when oauthProvider is also enabled.
                disableSettingJwtHeader: true,
                jwt: {
                    // Defaults for iss/aud are baseURL — same as oauthProvider,
                    // so first-party JWTs and OAuth access tokens validate
                    // with the exact same code path on the API.
                    expirationTime: "15m",
                    definePayload: ({ user }) => ({
                        // Only the fields routes actually read off c.get("user").
                        // Adding more bloats every request's Authorization header.
                        id: user.id,
                        email: user.email,
                        name: user.name,
                        image: user.image,
            ...
            oauthProvider({
                ...
                consentPage: "http://localhost:3000/oauth/consent",
