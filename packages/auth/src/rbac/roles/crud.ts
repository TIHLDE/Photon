 ...
 */
async function shiftRolesUp(fromPosition: number, tx: DbTransaction): Promise<void> {
 ...
 */
async function shiftRolesDown(deletedPosition: number, tx: DbTransaction): Promise<void> {
  const rolesToShift = await tx
    .select()
    .from(role)
 ...
 */
async function shiftRolesDownInRange(fromPosition: number, toPosition: number, tx: DbTransaction): Promise<void> {
  const rolesToShift = await tx
    .select()
    .from(role)
    .where(and(sql`${role.position} >= ${fromPosition + 1}`, sql`${role.position} < ${toPosition + 1}`))
    .orderBy(role.position);
 ...
 */
async function shiftRolesUpInRange(fromPosition: number, toPosition: number, tx: DbTransaction): Promise<void> {
  const rolesToShift = await tx
    .select()
    .from(role)
    .where(and(sql`${role.position} >= ${toPosition}`, sql`${role.position} < ${fromPosition}`))
    .orderBy(sql`${role.position} DESC`);
...
export async function createRole(
  ...
  const creatorPosition = await getUserHighestRolePosition(ctx, creatorUserId);
 ...
 */
export async function deleteRole(ctx: DbCtx, userId: string, roleId: number): Promise<void> {
  const [userPosition, targetRole] = await Promise.all([getUserHighestRolePosition(ctx, userId), getRoleById(ctx, roleId)]);
 ...
 */
export async function reorderRole(ctx: DbCtx, userId: string, roleId: number, newPosition: number): Promise<void> {
  const [userPosition, targetRole] = await Promise.all([getUserHighestRolePosition(ctx, userId), getRoleById(ctx, roleId)]);
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.update(role).set({ position: newPosition }).where(eq(role.id, roleId));
  });
}
