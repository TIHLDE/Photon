 ...
 */
export async function assignRolePermissions(ctx: DbCtx, roleName: string, permissionNames: string[]): Promise<void> {
 ...
 * Throws if role doesn't exist.
 */
export async function setRolePermissions(ctx: DbCtx, roleName: string, permissionNames: string[]): Promise<void> {
  const r = await getRoleByName(ctx, roleName);
  if (!r) throw new Error(`Role not found: ${roleName}`);

  const db = ctx.db;
  await db.update(role).set({ permissions: permissionNames }).where(eq(role.id, r.id));
}
