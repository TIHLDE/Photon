...
type ExtractPermissions<T extends PermissionNode, Prefix extends string = ""> = {
...
type PermissionFromRegistry = {
  [K in keyof Registry]: Registry[K] extends PermissionNode ? ExtractPermissions<Registry[K], `${K & string}:`> : never;
}[keyof Registry];
 ...
 */
function flattenPermissionRegistry(node: PermissionNode, prefix: string): string[] {
  const names: string[] = [];
  ...
  for (const key of Object.keys(node)) {
    ...
    if (key === "actions" && Array.isArray(value)) {
      ...
      names.push(...flattenPermissionRegistry(value as PermissionNode, `${prefix}${key}:`));
    }
  }
 ...
 */
export const PERMISSIONS: readonly string[] = Object.freeze([...flattenRegistry(PERMISSION_REGISTRY), ...SPECIAL_PERMISSIONS]);
