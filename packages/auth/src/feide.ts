import type { AuthContext, MiddlewareContext, MiddlewareOptions, OAuth2Tokens, OAuth2UserInfo } from "better-auth";
import { genericOAuth } from "better-auth/plugins";
import { eq } from "drizzle-orm";
import { account, studyProgram, studyProgramMembership } from "@photon/db/schema";
 ...
 */
const ALLOWED_PROGRAM_CODES = ["BIDATA", "ITBAITBEDR", "BDIGSEC", "ITMAIKTSA", "ITBAINFODR", "ITBAINFO"] as const;
...
export const syncFeideHook: (
  ...
  if (middlewareContext.path.startsWith("/oauth2/callback") && middlewareContext.params.providerId === FEIDE_PROVIDER_ID) {
    ...
    const feideAccount = await ctx.db.select({ accessToken: account.accessToken }).from(account).where(eq(account.userId, userId)).limit(1);
    ...
    await ctx.db.transaction(async (tx) => {
      for (const feideGroup of groups) {
        const sp = await tx.select({ id: studyProgram.id }).from(studyProgram).where(eq(studyProgram.feideCode, feideGroup.code)).limit(1);

        if (!sp[0]) {
          console.warn(`User is part of unknown study program ${feideGroup}, skipping`);
          continue;
        }
        ...
        const existing = await tx
          ...
          .where(eq(studyProgramMembership.userId, userId) && eq(studyProgramMembership.studyProgramId, studyProgramId))
          .limit(1);
 ...
 */
async function fetchValidStudyPrograms(accessToken: string): Promise<StudyProgram[]> {
  ...
  const response = await fetch("https://groups-api.dataporten.no/groups/me/groups", { headers: { Authorization: `Bearer ${accessToken}` } });
...
export function parseValidStudyPrograms(groups: FeideGroup[]): StudyProgram[] {
  return groups.flatMap((g) => {
    ...
    if (Number.isNaN(startYear) || startYear < 2000 || startYear > 3000) {
      throw new Error(`Invalid start year parsed from Feide: ${startYear}`);
    }

    return ALLOWED_PROGRAM_CODES.filter((p) => p === programCode).map((p) => ({
      code: p,
      startYear,
    }));
  });
