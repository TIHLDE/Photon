...
import { admin, bearer, customSession, emailOTP, openAPI, username } from "better-auth/plugins";
...
import { ChangeEmailVerificationEmail, OtpSignInEmail, ResetPasswordEmail } from "@photon/email/templates";
...
export const createAuth = (ctx: AuthCreateContext) =>
  betterAuth({
    ...
    trustedOrigins: ["https://tihlde.org", "https://*.tihlde.org", "localhost:3000", "http://localhost:3000"],
    user: {
      additionalFields: {
        /**
    ...
    plugins: [
      ...
      customSession(async ({ user, session }) => {
        ...
        const permissions = await getUserPermissions({ db: ctx.db }, user.id);
