...
function createEmailProcessor(ctx: EmailWorkerContext) {
  return async (job: Job<EmailJobData>): Promise<void> => {
    ...
    if (env.EMAIL_PROXY_URL) {
      await sendViaProxy({ to, subject, html }, env.EMAIL_PROXY_URL, env.EMAIL_PROXY_KEY ?? "");
 ...
 */
export function startEmailWorker(ctx: EmailWorkerContext): Worker<EmailJobData, void> {
  const worker = ctx.queue.createWorker<EmailJobData, void>(EMAIL_QUEUE_NAME, createEmailProcessor(ctx), {
  ...
  console.log(`📧 Email worker started (rate: 1 email per ${EMAIL_SEND_RATE_MS}ms)`);
