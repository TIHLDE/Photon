...
import { boolean, integer, pgTableCreator, primaryKey, serial, text, timestamp, uuid, varchar } from "drizzle-orm/pg-core";
...
export const studyProgramType = pgEnum("org_study_program_type", ["bachelor", "master"]);
...
export const groupType = pgEnum("org_group_type", ["studyyear", "interestgroup", "committee", "study", "private", "board", "subgroup", "tihlde"]);
 ...
 */
export const groupPermissionMode = pgEnum("org_group_permission_mode", ["leader_only", "member", "custom"]);

export type GroupPermissionMode = (typeof groupPermissionMode)["enumValues"][number];
...
export const group = pgTable("group", {
   ...
   */
  permissionMode: groupPermissionMode("permission_mode").notNull().default("leader_only"),
  ...timestamps,
});

export const groupMembershipRole = pgEnum("org_group_membership_role", ["member", "leader"]);

export type GroupMembershipRole = (typeof groupMembershipRole)["enumValues"][number];
...
export const fineStatus = pgEnum("org_fine_status", ["pending", "approved", "paid", "rejected"]);
