...
import { boolean, integer, pgEnum, pgTableCreator, primaryKey, text, timestamp, uuid, varchar } from "drizzle-orm/pg-core";
...
export const registrationStatus = pgEnum("event_registration_status", registrationStatusVariants);

export type RegistrationStatus = (typeof registrationStatusVariants)[number];

export const paymentStatus = pgEnum("event_payment_status", ["pending", "paid", "refunded", "failed"]);
...
export const event = pgTable("event", {
  ...
  isRegistrationClosed: boolean("is_registration_closed").default(false).notNull(),
...
export const eventPriorityPoolRelations = relations(eventPriorityPool, ({ one, many }) => ({
  event: one(event, {
...
export const eventPriorityPoolGroupRelations = relations(eventPriorityPoolGroup, ({ one }) => ({
