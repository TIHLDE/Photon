...
import { boolean, integer, pgEnum, pgTableCreator, text, unique, uuid, varchar } from "drizzle-orm/pg-core";
...
export const formFieldTypeVariants = ["text_answer", "multiple_select", "single_select"] as const;
...
export const eventFormType = pgEnum("form_event_form_type", eventFormTypeVariants);
...
export const formGroupForm = pgTable("group_form", {
  ...
  isOpenForSubmissions: boolean("is_open_for_submissions").default(false).notNull(),
  onlyForGroupMembers: boolean("only_for_group_members").default(false).notNull(),
  ...timestamps,
});
...
export const formAnswerOptionRelations = relations(formAnswerOption, ({ one }) => ({
