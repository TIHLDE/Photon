...
import { boolean, pgEnum, pgTableCreator, text, timestamp, uuid, varchar } from "drizzle-orm/pg-core";
import { timestamps } from "../timestamps";
import { user } from "./auth";

const pgTable = pgTableCreator((name) => `job_${name}`);

export const jobTypeVariants = ["full_time", "part_time", "summer_job", "other"] as const;

export const jobType = pgEnum("job_type", jobTypeVariants);

export type JobType = (typeof jobTypeVariants)[number];

export const userClassVariants = ["first", "second", "third", "fourth", "fifth", "alumni"] as const;
...
export const jobPost = pgTable("job_post", {
  ...
  isContinuouslyHiring: boolean("is_continuously_hiring").default(false).notNull(),
