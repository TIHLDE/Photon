...
const EventEditor = ({ eventId, goToEvent }: EventEditorProps) => {
  ...
  const setValues = useCallback(
    (newValues: Event | null) => {
      setPriorityPools(
        newValues?.priority_pools.map((pool) => ({
          groups: pool.groups.map((group) => group.slug),
        })) || [],
      );
  ...
  const getEventPreview = (): Event | null => {
    ...
    return {
      ...
      list_count: 0,
      priority_pools: priorityPools.map((pool) => ({
        groups: pool.groups.map((group) => groups?.find((g) => g.slug === group)),
      })) as Array<PriorityPool>,
