...
import { getJobQuery, getNewsQuery } from "~/api/queries";
...
export function EventCard({ id }: { id: string }) {
  const { data } = useSuspenseQuery(getEventQuery(id));
  return <EventListItem event={data} size="medium" />;
}

export function JobPostCard({ id }: { id: string }) {
  const { data } = useSuspenseQuery();
  return <JobPostListItem jobPost={data} />;
}

export function NewsCard({ id }: { id: string }) {
  const { data } = useSuspenseQuery(getNewsQuery(id));
  return <NewsListItem news={data} />;
}
...
export const CodeBlock = ({ inline = false, className: language, children }: CodeBlockProps) => {
  ...
  if (inline) {
    ...
    if (language === LanguageTypes.EVENT) {
      return (
        <ErrorBoundary fallback={<EventListLoading length={1} />}>
          <Suspense fallback={<EventListLoading length={1} />}>
            <EventCard id={String(id)} />
          </Suspense>
        </ErrorBoundary>
      );
    } else if (language === LanguageTypes.JOBPOST) {
      return (
        <ErrorBoundary fallback={<JobPostListItemLoading length={1} />}>
          <Suspense fallback={<JobPostListItemLoading length={1} />}>
            <JobPostCard id={id} />;
          </Suspense>
        </ErrorBoundary>
      );
      ...
      return (
        <ErrorBoundary fallback={<NewsListItemLoading length={1} />}>
          <Suspense fallback={<NewsListItemLoading length={1} />}>
            <NewsCard id={id} />
          </Suspense>
        </ErrorBoundary>
      );
