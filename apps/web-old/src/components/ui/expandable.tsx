...
const Expandable = ({ title, description, icon, extra, children, className, open, onOpenChange }: ExpandableProps) => {
  ...
  return (
    ...
    >
        ...
        >
          <div className="flex items-center space-x-2 md:space-x-4 w-full overflow-hidden">
            ...
            <div className="text-start break-words">
              {typeof title === "string" ? <h1 className="text-sm md:text-base">{title}</h1> : title}
              {typeof description === "string" ? <h1 className="text-xs md:text-sm">{description}</h1> : description}
            </div>
          </div>
      ...
      <CollapsibleContent className="border border-t-secondary border-b-0 border-x-0 *:p-2 md:*:p-4">{children}</CollapsibleContent>
    </Collapsible>
  );
};
