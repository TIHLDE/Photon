...
export default async ({ db }: AppContext) => {
  const groups = [
    ...
    },
    {
      ...
      name: "Index",
      slug: "index",
      description: "Index jobber smidig med utviklingen av linjeforeningens løsninger, blant annet nettsiden du er inne på akkurat nå.",
  ...
  for (const group of groups) {
    const exists = await db.select().from(schema.group).where(eq(schema.group.slug, group.slug)).limit(1);
    if (!exists.length) {
