...
export default async ({ db }: AppContext) => {
  const allergies = [
    {
      ...
      description: "Hvete, rug, bygg, havre, spelt, kamut/egyptisk hvete eller hybrider av disse",
    },
    {
      slug: "shellfish",
      label: "Skalldyr",
    ...
    },
    {
      slug: "nuts",
      label: "Nøtter",
      description: "Nøtter (mandel, hasselnøtt, valnøtt, cashewnøtt, pekannøtt, paranøtt, pistasjnøtt, macadamianøtt/australianøtt)",
    },
    {
      slug: "celery",
      label: "Selleri",
  ...
  for (const allergy of allergies) {
    const exists = await db.select().from(schema.allergy).where(eq(schema.allergy.slug, allergy.slug)).limit(1);
