...
export default async ({ db }: AppContext) => {
  ...
  for (const category of eventCategories) {
    const exists = await db.select().from(schema.eventCategory).where(eq(schema.eventCategory.slug, category.slug)).limit(1);
    ...
    const eventsExists = await db.select().from(schema.event).where(eq(schema.event.categorySlug, category.slug)).limit(1);
