  ...
  describe("remove group member", () => {
    integrationTest(
      ...
      async ({ ctx }) => {
        ...
        // Remove member
        const response = await client.api.groups[":groupSlug"].members[":userId"].$delete({
          param: {
            groupSlug: group.slug,
            userId: memberUser.user.id,
          },
        ...
        const membership = await ctx.db.query.groupMembership.findFirst({
          where: (m, { and, eq }) => and(eq(m.userId, memberUser.user.id), eq(m.groupSlug, group.slug)),
        });
  ...
  describe("update group member", () => {
    integrationTest(
      ...
      async ({ ctx }) => {
        ...
        // Update to leader
        const response = await client.api.groups[":groupSlug"].members[":userId"].$patch({
          param: {
            groupSlug: group.slug,
            userId: memberUser.user.id,
          },
