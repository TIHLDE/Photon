...
describe("Asset Upload/Download System", () => {
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      const testFileContent = Buffer.concat([Buffer.from(pngHeader), Buffer.from("test image content")]);

      // ===== TEST 1: UPLOAD WITH SESSION AUTH =====
      const formData = new FormData();
      formData.append("file", new Blob([testFileContent], { type: "image/png" }), "test-image.png");
      ...
      const testKey = await bucket.upload("uploads/2026/01/test-uuid_test-image.png", testFileContent, {
      ...
      const downloadResponse = await app.request(`/api/assets/${testKey}`);
      expect(downloadResponse.status).toBe(200);
      expect(downloadResponse.headers.get("Content-Type")).toBe("image/png");
      expect(downloadResponse.headers.get("Cache-Control")).toBe("public, max-age=31536000, immutable");

      const downloadedContent = await downloadResponse.arrayBuffer();
      expect(Buffer.from(downloadedContent)).toEqual(testFileContent);

      // ===== TEST 4: GET METADATA =====
      const metadataResponse = await app.request(`/api/assets/metadata/${testKey}`);
      ...
      const promotedMetadataResponse = await app.request(`/api/assets/metadata/${testKey}`);
      const promotedMetadata = (await promotedMetadataResponse.json()) as any;
      ...
      const notFoundResponse = await app.request("/api/assets/uploads/2026/01/nonexistent.png");
      expect(notFoundResponse.status).toBe(404);

      // ===== TEST 8: METADATA FOR NON-EXISTENT FILE =====
      const notFoundMetadataResponse = await app.request("/api/assets/metadata/uploads/2026/01/nonexistent.png");
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      const formData = new FormData();
      formData.append("file", new Blob([testFileContent], { type: "image/png" }), "test.png");
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      const { createApiKeyService } = await import("~/lib/service/api-key");
      ...
      const formData = new FormData();
      formData.append("file", new Blob([testFileContent], { type: "image/jpeg" }), "api-upload.jpg");
      ...
      expect(result.key).toMatch(/^uploads\/\d{4}\/\d{2}\/.+_api-upload\.jpg$/);
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      for (const { type, ext } of validTypes) {
        const formData = new FormData();
        formData.append("file", new Blob([Buffer.from(`test ${type} content`)], { type }), `test.${ext}`);
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      await bucket.upload(oldStagedKey, Buffer.from("old staged content"), {
      ...
      await db.update(schema.asset).set({ createdAt: threeDaysAgo }).where(eq(schema.asset.key, oldStagedKey));
      ...
      await bucket.upload(oldPromotedKey, Buffer.from("old promoted content"), {
        originalFilename: "old_promoted.png",
        contentType: "image/png",
        uploadedById: testUser.id,
      });
      await bucket.promoteAsset(oldPromotedKey);
      await db.update(schema.asset).set({ createdAt: threeDaysAgo }).where(eq(schema.asset.key, oldPromotedKey));
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      const result = await bucket.promoteAsset("uploads/2026/01/nonexistent.png");
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        // Verify format
        expect(key).toMatch(/^uploads\/\d{4}\/\d{2}\/[a-f0-9-]+_test-file\.png$/);
      }
    },
    60_000,
  );
