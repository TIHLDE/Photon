...
describe("Job Postings System", () => {
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      await ctx.utils.giveUserPermissions(recruiter, ["jobs:create", "jobs:update"]);
      ...
      const invalidClassRangeResponse = await recruiterClient.api.jobs.$post({
        json: {
      ...
      expect(jobsList.items.find((j) => j.id === expiredJob.id)).toBeUndefined();
      ...
      expect(allJobs.items.find((j) => j.id === expiredJob.id)).toBeDefined();

      // 8. Verify expired field is computed correctly
      const activeJobInList = allJobs.items.find((j) => j.id === activeJob.id);
      expect(activeJobInList?.expired).toBe(false);

      const expiredJobInList = allJobs.items.find((j) => j.id === expiredJob.id);
      ...
      expect(searchTitleResults.items.every((j) => j.title.includes("Developer"))).toBe(true);
      ...
      const posterUpdateResponse = await posterClient.api.jobs[":id"].$patch({
        param: { id: activeJob.id },
        json: {
      ...
      expect(updatedByPoster.title).toBe("Updated Senior Developer Position");
      expect(updatedByPoster.location).toBe("Oslo / Remote");

      // 14. Recruiter with global permission can update any job
      const recruiterUpdateResponse = await recruiterClient.api.jobs[":id"].$patch({
        param: { id: activeJob.id },
        json: {
      ...
      const unauthorizedUpdateResponse = await userClient.api.jobs[":id"].$patch({
        param: { id: activeJob.id },
        json: {
          title: "Should fail",
        },
      ...
      const validClassUpdateResponse = await posterClient.api.jobs[":id"].$patch({
        param: { id: activeJob.id },
        json: {
          classStart: "first",
          classEnd: "alumni",
      ...
      const invalidClassUpdateResponse = await posterClient.api.jobs[":id"].$patch({
        param: { id: activeJob.id },
        json: {
          classStart: "fifth",
          classEnd: "second",
      ...
      const updateClassStartResponse = await posterClient.api.jobs[":id"].$patch({
        param: { id: activeJob.id },
        json: {
          classStart: "second",
        },
      ...
      const invalidClassStartResponse = await recruiterClient.api.jobs[":id"].$patch({
      ...
      const scopedUpdateResponse = await userClient.api.jobs[":id"].$patch({
      ...
      const scopedUpdateFailResponse = await userClient.api.jobs[":id"].$patch({
        param: { id: activeJob.id },
        json: {
      ...
      const unauthorizedDeleteResponse = await userClient.api.jobs[":id"].$delete({
        param: { id: expiredJob.id },
      });
      ...
      const posterDeleteResponse = await posterClient.api.jobs[":id"].$delete({
        param: { id: activeJob.id },
      });
      ...
      const adminDeleteResponse = await adminClient.api.jobs[":id"].$delete({
        param: { id: expiredJob.id },
      });
      ...
      const scopedDeleteResponse = await userClient.api.jobs[":id"].$delete({
