  ...
  describe("Basic Registration", () => {
    integrationTest(
      ...
      async ({ ctx }) => {
        ...
        const registration = await ctx.db.query.eventRegistration.findFirst({
          where: (reg, { and, eq }) => and(eq(reg.eventId, event.id), eq(reg.userId, user.id)),
        });
    ...
    integrationTest(
      ...
      async ({ ctx }) => {
        ...
        const reg1 = await ctx.db.query.eventRegistration.findFirst({
          where: (reg, { and, eq }) => and(eq(reg.eventId, event.id), eq(reg.userId, user1.id)),
        });
        ...
        const reg2 = await ctx.db.query.eventRegistration.findFirst({
          where: (reg, { and, eq }) => and(eq(reg.eventId, event.id), eq(reg.userId, user2.id)),
        });
    ...
    integrationTest(
      ...
      async ({ ctx }) => {
        ...
        const end = Date.now();
        console.log(`Created 100 users and registrations in ${end - start}ms`);
  ...
  describe("Priority Pool System", () => {
    integrationTest(
      ...
      async ({ ctx }) => {
        ...
        const [pool] = await ctx.db.insert(schema.eventPriorityPool).values({ eventId: event.id, priorityScore: 1 }).returning();
        ...
        const nonPriorityReg1 = await ctx.db.query.eventRegistration.findFirst({
          where: (reg, { and, eq }) => and(eq(reg.eventId, event.id), eq(reg.userId, nonPrioritizedUser.id)),
        });
        ...
        const priorityReg = await ctx.db.query.eventRegistration.findFirst({
          where: (reg, { and, eq }) => and(eq(reg.eventId, event.id), eq(reg.userId, prioritizedUser.id)),
        });
    ...
    integrationTest(
      ...
      async ({ ctx }) => {
        ...
        await ctx.utils.createPendingRegistration(event.id, firstUserData.user.id);
        ...
        const partialReg = await ctx.db.query.eventRegistration.findFirst({
          where: (reg, { and, eq }) => and(eq(reg.eventId, event.id), eq(reg.userId, partialUser.id)),
        });
    ...
    integrationTest(
      ...
      async ({ ctx }) => {
        ...
        const strikedReg = await ctx.db.query.eventRegistration.findFirst({
          where: (reg, { and, eq }) => and(eq(reg.eventId, event.id), eq(reg.userId, strikedUserData.user.id)),
        });
  ...
  describe("Waitlist Position Calculation", () => {
    integrationTest(
      ...
      async ({ ctx }) => {
        ...
        const reg2 = await ctx.db.query.eventRegistration.findFirst({
          where: (reg, { and, eq }) => and(eq(reg.eventId, event.id), eq(reg.userId, user2Data.user.id)),
        });
        const reg3 = await ctx.db.query.eventRegistration.findFirst({
          where: (reg, { and, eq }) => and(eq(reg.eventId, event.id), eq(reg.userId, user3Data.user.id)),
        });
        const reg4 = await ctx.db.query.eventRegistration.findFirst({
          where: (reg, { and, eq }) => and(eq(reg.eventId, event.id), eq(reg.userId, user4Data.user.id)),
        });
