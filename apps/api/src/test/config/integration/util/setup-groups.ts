...
export const createSetupGroups = (ctx: TestUtilContext) => async () => {
  const groups = [
    {
    ...
    },
    {
      ...
      name: "Redaksjonen",
      slug: "redaksjonen",
      description: "Redaksjonen er gjengen som produserer linjeforeningsbladet til TIHLDE.",
    ...
    },
    {
      ...
      name: "Sosialen",
      slug: "sosialen",
      description: "Sosialen sørger for samhold på tvers av alle linjene i TIHLDE! De arrangerer alt fra LAN til fester og hytteturer.",
    ...
    },
    {
      ...
      type: "INTERESTGROUP",
      fine_info: "alt kan være en bot, vær kreativ, botfest en gang i semesteren\n\nBotsjef: Erik Hoff",
      fines_activated: 1,
  ...
  for (const group of groups) {
    const exists = await ctx.db.select().from(schema.group).where(eq(schema.group.slug, group.slug)).limit(1);
    if (!exists.length) {
      ...
      await ctx.db.insert(schema.group).values(newGroup);
    }
  }
};
