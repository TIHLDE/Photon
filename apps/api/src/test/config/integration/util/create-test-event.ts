...
export const createCreateTestEvent = (ctx: TestUtilContext) => async (overrides?: Partial<EventInsert>) => {
  const now = Date.now();
  ...
  const [event] = await ctx.db.insert(schema.event).values(defaults).returning();
