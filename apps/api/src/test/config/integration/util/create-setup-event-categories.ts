...
export const createSetupEventCategories = (ctx: TestUtilContext) => async () => {
  ...
  for (const category of eventCategories) {
    const exists = await ctx.db.select().from(schema.eventCategory).where(eq(schema.eventCategory.slug, category.slug)).limit(1);
