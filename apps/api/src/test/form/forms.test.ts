...
describe("Forms System", () => {
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      await ctx.utils.giveUserPermissions(admin, ["forms:create", "forms:manage", "events:create"]);
      await ctx.utils.giveUserPermissions(eventOrganizer, ["events:create"]);

      await ctx.utils.setupGroups();
      await ctx.utils.setupEventCategories();

      const adminClient = await ctx.utils.clientForUser(admin);
      const organizerClient = await ctx.utils.clientForUser(eventOrganizer);
      ...
      const surveyResponse = await organizerClient.api.event[":eventId"].forms.$post({
        param: { eventId },
        json: {
      ...
      const evalResponse = await organizerClient.api.event[":eventId"].forms.$post({
        param: { eventId },
        json: {
      ...
      // List event forms
      const eventFormsResponse = await userClient.api.event[":eventId"].forms.$get({
        param: { eventId },
      });
      ...
      const surveySubmitResponse = await userClient.api.forms[":formId"].submissions.$post({
      ...
      const groupFormResponse = await leaderClient.api.groups[":slug"].forms.$post({
        param: { slug: "index" },
        json: {
      ...
      const nonMemberSubmitResponse = await userClient.api.forms[":formId"].submissions.$post({
      ...
      const memberSubmitResponse = await userClient.api.forms[":formId"].submissions.$post({
        param: { formId: groupForm.id! },
        json: {
          answers: [
            {
      ...
      const duplicateSubmitResponse = await userClient.api.forms[":formId"].submissions.$post({
        param: { formId: groupForm.id! },
        json: {
          answers: [
            {
            ...
            answers: [
              {
                ...
                selected_options: [
                  ...
                  {
                    id: statsForm.fields?.[0]?.options?.[1]?.id!,
                  }, // Blue
                ],
              },
            ],
      ...
      // Get statistics
      const statsResponse = await adminClient.api.forms[":id"].statistics.$get({
      ...
      expect(stats.statistics?.[0]?.options?.[0]?.answer_percentage).toBe(100);
      ...
      const submissionsResponse = await adminClient.api.forms[":formId"].submissions.$get({
        param: { formId: statsForm.id! },
      });
      ...
      const unauthorizedListResponse = await userClient.api.forms[":formId"].submissions.$get({
        param: { formId: statsForm.id! },
      });
      ...
      const answeredForm = allForms.find((f: any) => f.id === statsForm.id);
      expect(answeredForm?.viewer_has_answered).toBe(true);

      // 10. Test form field validation
      const invalidSubmitResponse = await userClient.api.forms[":formId"].submissions.$post({
        param: { formId: statsForm.id! },
        json: {
          answers: [
            {
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      const nonAttendeeClient = await ctx.utils.clientForUser(nonAttendee);
      ...
      const attendeeSubmitResponse = await attendeeClient.api.forms[":formId"].submissions.$post({
          ...
          answers: [
            {
              ...
              selected_options: [
                {
                  id: evaluation.fields?.[0]?.options?.[0]?.id!,
                },
              ],
            },
          ],
      ...
      const nonAttendeeSubmitResponse = await nonAttendeeClient.api.forms[":formId"].submissions.$post({
        param: { formId: evaluation.id! },
        json: {
          answers: [
            {
