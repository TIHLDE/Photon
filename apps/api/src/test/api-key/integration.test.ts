...
integrationTest(
  ...
  async ({ ctx }) => {
    ...
    await ctx.utils.giveUserPermissions(testUser, ["api-keys:view", "api-keys:create", "api-keys:update", "api-keys:delete"]);
    ...
    const invalidValidateResponse = await client.api["api-keys"].validate.$post({
      json: {
    ...
    const regenerateResponse = await client.api["api-keys"][":id"].regenerate.$post({
      param: { id: apiKeyId },
    });
    ...
    const deletedKeyValidation = await client.api["api-keys"].validate.$post({
      json: {
        key: regeneratedKey.key,
      },
    });
    ...
    const unauthorizedClient = await ctx.utils.clientForUser(unauthorizedUser);

    const unauthorizedCreateResponse = await unauthorizedClient.api["api-keys"].$post({
      json: {
    ...
    const unauthenticatedResponse = await unauthenticatedClient.api["api-keys"].$get();
