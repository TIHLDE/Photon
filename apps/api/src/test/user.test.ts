...
describe("user endpoints", () => {
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      expect(json.message).toBe("User settings not found. Please complete onboarding.");
    },
    500_000,
  );
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      expect(userAllergies.map((ua) => ua.allergySlug)).toEqual(expect.arrayContaining(["lactose", "nuts"]));
    },
    500_000,
  );
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      expect(json.message).toBe("User settings not found. Please complete onboarding first.");
    },
    500_000,
  );
  ...
  integrationTest(
    ...
    async ({ ctx }) => {
      ...
      const json = await response.json();
      expect(json.allergies).toEqual(expect.arrayContaining(["gluten", "nuts"]));
      ...
      expect(userAllergies.length).toBe(2);
      expect(userAllergies.map((ua) => ua.allergySlug)).toEqual(expect.arrayContaining(["gluten", "nuts"]));
    },
    500_000,
  );
