...
function getVippsClient() {
  if (!env.VIPPS_MERCHANT_SERIAL_NUMBER || !env.VIPPS_SUBSCRIPTION_KEY || !env.VIPPS_CLIENT_ID || !env.VIPPS_CLIENT_SECRET) {
    throw new Error(
...
async function getVippsToken(): Promise<string> {
  ...
  // Fetch new token
  const tokenResponse = await vipps.auth.getToken(env.VIPPS_CLIENT_ID || "", env.VIPPS_CLIENT_SECRET || "");

  if (!tokenResponse.ok) {
    throw new Error(`Failed to get Vipps token: ${tokenResponse.error instanceof Error ? tokenResponse.error.message : "Unknown error"}`);
  }
 ...
 */
export async function getPaymentDetails(reference: string): Promise<GetPaymentResponse> {
  const token = await getVippsToken();
  const vipps = getVippsClient();
...
export async function setupWebhooks(): Promise<{ id: string; secret: string }> {
  ...
  if (existingWebhookId) {
    if (!env.REFRESH_VIPPS_WEBHOOKS) {
      ...
      hasRegisteredWebhook = response.data.webhooks.some((w) => w.id === existingWebhookId);
    }
  }
