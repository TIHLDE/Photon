...
export async function getFine(ctx: AppContext, fineId: string) {
  const db = ctx.db;
  const [fine] = await db.select().from(schema.fine).where(eq(schema.fine.id, fineId)).limit(1);
 ...
 */
export async function canCreateFine(ctx: AppContext, userId: string, groupSlug: string): Promise<boolean> {
 ...
 */
export async function canApproveFine(ctx: AppContext, userId: string, fineId: string): Promise<boolean> {
  // Root or fines:manage bypass
  if (await hasPermission(ctx, userId, ["root", "fines:manage"])) {
    return true;
  }
  ...
  const [isLeader, hasUpdatePerm] = await Promise.all([isGroupLeader(ctx, userId, fine.groupSlug), hasPermission(ctx, userId, "fines:update")]);
 ...
 * @param fineId - Fine ID
 */
export async function canDeleteFine(ctx: AppContext, userId: string, fineId: string): Promise<boolean> {
  // Root or fines:manage bypass
  if (await hasPermission(ctx, userId, ["root", "fines:manage"])) {
    return true;
  }
  ...
  const [isLeader, hasDeletePerm] = await Promise.all([isGroupLeader(ctx, userId, fine.groupSlug), hasPermission(ctx, userId, "fines:delete")]);
 ...
 * @param fineId - Fine ID
 */
export async function canViewFine(ctx: AppContext, userId: string, fineId: string): Promise<boolean> {
  ...
  const [membership, hasViewPerm] = await Promise.all([
    ctx.db.query.groupMembership.findFirst({
      where: (gm, { and, eq }) => and(eq(gm.userId, userId), eq(gm.groupSlug, fine.groupSlug)),
    }),
 ...
 * @param fineId - Fine ID
 */
export async function canAddFineDefense(ctx: AppContext, userId: string, fineId: string): Promise<boolean> {
  const fine = await getFine(ctx, fineId);
  if (!fine) {
    return false; // Fine doesn't exist
  }
 ...
 * @param fineId - Fine ID
 */
export async function canMarkFinePaid(ctx: AppContext, userId: string, fineId: string): Promise<boolean> {
...
export async function getUserFines(ctx: AppContext, userId: string) {
  const db = ctx.db;
  return await db.select().from(schema.fine).where(eq(schema.fine.userId, userId));
}
 ...
 */
export async function getGroupFines(ctx: AppContext, userId: string, groupSlug: string) {
  // Check if user can view fines for this group
  const [membership, hasViewPerm] = await Promise.all([
    ctx.db.query.groupMembership.findFirst({
      where: (gm, { and, eq }) => and(eq(gm.userId, userId), eq(gm.groupSlug, groupSlug)),
    }),
    hasPermission(ctx, userId, "fines:view"),
  ]);
  ...
  const db = ctx.db;
  return await db.select().from(schema.fine).where(eq(schema.fine.groupSlug, groupSlug));
}
