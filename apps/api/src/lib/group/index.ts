...
import { assignUserRole, getRoleById, removeUserRole } from "@photon/auth/roles";
 ...
 */
export async function getGroupMembership(ctx: AppContext, userId: string, groupSlug: string) {
  const db = ctx.db;
  const [membership] = await db
    .select()
    .from(schema.groupMembership)
    .where(and(eq(schema.groupMembership.userId, userId), eq(schema.groupMembership.groupSlug, groupSlug)))
    .limit(1);
 ...
 */
export async function isGroupMember(ctx: AppContext, userId: string, groupSlug: string): Promise<boolean> {
 ...
 */
export async function isGroupLeader(ctx: AppContext, userId: string, groupSlug: string): Promise<boolean> {
  const membership = await getGroupMembership(ctx, userId, groupSlug);
...
export async function getGroup(ctx: AppContext, groupSlug: string) {
  const db = ctx.db;
  const [group] = await db.select().from(schema.group).where(eq(schema.group.slug, groupSlug)).limit(1);
 ...
 */
export async function addUserToGroup(ctx: AppContext, userId: string, groupSlug: string, role: "member" | "leader" = "member"): Promise<void> {
  const db = ctx.db;
 ...
 * @param groupSlug - Group slug
 */
export async function removeUserFromGroup(ctx: AppContext, userId: string, groupSlug: string): Promise<void> {
  const db = ctx.db;
  ...
  await db.delete(schema.groupMembership).where(and(eq(schema.groupMembership.userId, userId), eq(schema.groupMembership.groupSlug, groupSlug)));
 ...
 */
export async function updateGroupMemberRole(ctx: AppContext, userId: string, groupSlug: string, newRole: "member" | "leader"): Promise<void> {
  const db = ctx.db;
 ...
 * }
 */
export async function canManageGroupResource(ctx: AppContext, userId: string, groupSlug: string, permission: string): Promise<boolean> {
 ...
 */
export async function canManageGroup(ctx: AppContext, userId: string, groupSlug: string): Promise<boolean> {
  ...
  const [isLeader, hasUpdatePerm] = await Promise.all([isGroupLeader(ctx, userId, groupSlug), hasPermission(ctx, userId, "groups:update")]);
 ...
 */
export async function getUserGroups(ctx: AppContext, userId: string): Promise<Array<{ slug: string; role: "member" | "leader" }>> {
  const db = ctx.db;
...
> {
  const db = ctx.db;
  const members = await db.select().from(schema.groupMembership).where(eq(schema.groupMembership.groupSlug, groupSlug));
