 ...
 */
export const API_KEY_PERMISSIONS_SET = new Set<string>(API_KEY_PERMISSIONS as readonly string[]);
 ...
 */
export function validateApiKeyPermissions(permissions: string[]): ApiKeyPermission[] {
  const invalid = permissions.filter((p) => !isApiKeyPermission(p));
  if (invalid.length > 0) {
    throw new ValidationError(`Invalid API key permissions: ${invalid.join(", ")}. Valid permissions are: ${Array.from(API_KEY_PERMISSIONS).join(", ")}`, {
...
function generateApiKey(): string {
  ...
  const base64url = randomBytesBuffer.toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
...
function parsePermissions(permissionsJson: string): ApiKeyPermission[] {
  try {
    ...
    throw new Error(`Failed to parse API key permissions: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function parseMetadata(metadataJson: string | null): Record<string, unknown> | null {
  ...
  try {
    ...
    throw new Error(`Failed to parse API key metadata: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export function createApiKeyService(ctx: AppContext): ApiKeyService {
  ...
  return {
    ...
    async create(params: CreateApiKeyParams): Promise<CreateApiKeyResult> {
      ...
      const validatedPermissions = validateApiKeyPermissions(params.permissions);
    ...
    async getById(id: string): Promise<ApiKey | null> {
      const [row] = await db.select().from(apiKeyTable).where(eq(apiKeyTable.id, id)).limit(1);
    ...
    async update(id: string, params: UpdateApiKeyParams): Promise<ApiKey> {
      ...
      if (params.metadata !== undefined) {
        updates.metadata = params.metadata ? JSON.stringify(params.metadata) : null;
      }

      const [updated] = await db.update(apiKeyTable).set(updates).where(eq(apiKeyTable.id, id)).returning();
    ...
    async regenerate(id: string): Promise<CreateApiKeyResult> {
      ...
      // Update in database
      const [updated] = await db.update(apiKeyTable).set({ keyHash, keyPrefix }).where(eq(apiKeyTable.id, id)).returning();
    ...
    async validate(key: string): Promise<ValidateApiKeyResult> {
      ...
      // Look up by hash
      const [row] = await db.select().from(apiKeyTable).where(eq(apiKeyTable.keyHash, keyHash)).limit(1);
      ...
      // Update lastUsedAt
      const [updated] = await db.update(apiKeyTable).set({ lastUsedAt: new Date() }).where(eq(apiKeyTable.id, row.id)).returning();
