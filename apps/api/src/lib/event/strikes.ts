 ...
 */
export async function getUserStrikeCount(userId: string, db: NodePgDatabase<DbSchema>): Promise<number> {
 ...
 */
export function canRegisterBasedOnStrikes(strikeCount: number, registrationStart: Date | null, pendingCreatedAt: Date): CanRegisterResult {
  if (strikeCount === 0 || !registrationStart) {
    return { allowed: true };
  }

  const hoursToWait = strikeCount === 1 ? 3 : 12;
  const allowedTime = new Date(registrationStart.getTime() + hoursToWait * 60 * 60 * 1000);
