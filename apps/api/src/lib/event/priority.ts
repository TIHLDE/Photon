 ...
 */
export async function getUserGroupSlugs(userId: string, db: NodePgDatabase<DbSchema>): Promise<Set<string>> {
 ...
 */
export function isUserPrioritized({ userGroupSlugs, eventPools, strikeCount, enforcesPreviousStrikes }: IsUserPrioritizedParams): boolean {
...
export async function findSwapTarget(
  ...
  const registered = registeredUsers.filter((r) => r.status === "registered").sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());

  // Find the first non-prioritized user
  for (const reg of registered) {
    const [userGroupSlugs, strikeCount] = await Promise.all([getUserGroupSlugs(reg.userId, db), getUserStrikeCount(reg.userId, db)]);
...
async function getUserStrikeCount(userId: string, db: NodePgDatabase<DbSchema>): Promise<number> {
...
export async function calculateWaitlistPosition(
  ...
  const waitlisted = await db.query.eventRegistration.findMany({
    where: (reg, { and, eq }) => and(eq(reg.eventId, eventId), eq(reg.status, "waitlisted")),
  ...
  throw new Error(`User ${userId} not found in waitlist for event ${eventId}`);
}
