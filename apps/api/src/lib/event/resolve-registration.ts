...
import { RegistrationBlockedEmail, RegistrationConfirmedEmail, SwappedToWaitlistEmail, WaitlistPlacementEmail } from "@photon/email/templates";
import { and, eq } from "drizzle-orm";
import type { AppContext } from "../ctx";
import { env } from "../env";
import { sendNotification } from "../notification";
import { calculateWaitlistPosition, findSwapTarget, getUserGroupSlugs, isUserPrioritized } from "./priority";
 ...
 */
export async function resolveRegistrationsForEvent(eventId: string, ctx: AppContext): Promise<void> {
  ...
  await ctx.db.transaction(async (tx) => {
    ...
    const pendingRegistrations = await tx
      ...
      .where(and(eq(schema.eventRegistration.eventId, eventId), eq(schema.eventRegistration.status, "pending")))
    ...
    if (!event.requiresSigningUp || event.isRegistrationClosed) {
      ...
      for (const registration of pendingRegistrations) {
        await tx
          ...
          .where(and(eq(schema.eventRegistration.eventId, eventId), eq(schema.eventRegistration.userId, registration.userId)));
      }
      return;
    }
    ...
    let availableSpots = isUnlimitedCapacity ? Number.POSITIVE_INFINITY : Math.max(0, (event.capacity ?? 0) - registeredCount);
    ...
    for (const registration of pendingRegistrations) {
      ...
      const [userGroupSlugs, strikeCount] = await Promise.all([getUserGroupSlugs(userId, tx), getUserStrikeCount(userId, tx)]);

      // Check strike-based timing restriction
      const { allowed, reason } = canRegisterBasedOnStrikes(strikeCount, event.registrationStart, createdAt);
      ...
      if (!allowed) {
        ...
        await tx
          .update(schema.eventRegistration)
          .set({ status: "cancelled" })
          .where(and(eq(schema.eventRegistration.eventId, eventId), eq(schema.eventRegistration.userId, userId)));
      ...
      if (isUnlimitedCapacity || availableSpots > 0) {
        ...
        const swapTarget = await findSwapTarget(event.registrations, event.pools, event.enforcesPreviousStrikes, tx);
        ...
        if (swapTarget) {
          ...
          await tx
            ...
            .where(and(eq(schema.eventRegistration.eventId, eventId), eq(schema.eventRegistration.userId, swapTarget.userId)));
          ...
          console.log(`User ${swapTarget.userId} swapped to waitlist by prioritized user ${userId}`);
        } else {
      ...
      if (finalStatus === "waitlisted") {
        waitlistPosition = await calculateWaitlistPosition(userId, eventId, event.pools, event.enforcesPreviousStrikes, tx);
      ...
      console.log(`User ${userId} resolved to status: ${finalStatus}${waitlistPosition ? ` (position ${waitlistPosition})` : ""}`);
      ...
      if (shouldRecalculateWaitlist) {
        const waitlisted = await tx.query.eventRegistration.findMany({
          where: (r, { and, eq }) => and(eq(r.eventId, eventId), eq(r.status, "waitlisted")),
        });

        for (const wReg of waitlisted) {
          const newPosition = await calculateWaitlistPosition(wReg.userId, eventId, event.pools, event.enforcesPreviousStrikes, tx);
          await tx
            .update(schema.eventRegistration)
            .set({ waitlistPosition: newPosition })
            .where(and(eq(schema.eventRegistration.eventId, eventId), eq(schema.eventRegistration.userId, wReg.userId)));
      ...
      if (swappedUserId) {
        const swappedIndex = event.registrations.findIndex((r) => r.userId === swappedUserId);
        if (swappedIndex !== -1) {
