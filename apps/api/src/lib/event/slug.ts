 ...
 */
export const generateUniqueEventSlug = async (title: string, transaction: DbTransaction): Promise<string> => {
  let isUnique = false;
  let slug = title
    .toLowerCase()
  ...
  while (!isUnique) {
    const [existingEvent] = await transaction.select().from(schema.event).where(eq(schema.event.slug, slug)).limit(1);
