...
export interface StorageClient {
   ...
   */
  upload: (key: string, body: Buffer | string, metadata: UploadMetadata) => Promise<string>;
   ...
   */
  listAssets: (options?: { uploadedById?: string; limit?: number; offset?: number }) => Promise<schema.Asset[]>;
...
export async function createStorageClient(options?: {
  ...
  const upload = async (key: string, body: Buffer | string, metadata: UploadMetadata): Promise<string> => {
  ...
  const getAsset = async (key: string): Promise<schema.Asset | null> => {
    if (!db) {
      throw new Error("Database instance required for getAsset operation");
    }
  ...
  const listAssets = async (options?: { uploadedById?: string; limit?: number; offset?: number }): Promise<schema.Asset[]> => {
    if (!db) {
      throw new Error("Database instance required for listAssets operation");
    }

    const query = db.query.asset.findMany({
      where: options?.uploadedById ? eq(schema.asset.uploadedById, options.uploadedById) : undefined,
  ...
  const promoteAsset = async (key: string): Promise<schema.Asset | null> => {
    if (!db) {
      throw new Error("Database instance required for promoteAsset operation");
    }
