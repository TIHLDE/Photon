...
export const ALLOWED_MIME_TYPES = ["image/jpeg", "image/png", "image/gif", "image/webp", "application/pdf"] as const;
 ...
 */
export async function getStagedAssetsForCleanup(db: NodePgDatabase<DbSchema>, cutoffDate: Date, limit = 100): Promise<schema.Asset[]> {
  return db.query.asset.findMany({
    where: and(eq(schema.asset.status, "staged"), lt(schema.asset.createdAt, cutoffDate)),
    limit,
 ...
 */
export async function deleteAsset(bucket: StorageClient, key: string): Promise<void> {
