...
import type { CreateAnswerInput, CreateFieldInput, UpdateFieldInput } from "~/routes/form/schema";
import {
 ...
 */
export async function userHasSubmitted(db: Database, formId: string, userId: string): Promise<boolean> {
  const submission = await db.query.formSubmission.findFirst({
    where: and(eq(schema.formSubmission.formId, formId), eq(schema.formSubmission.userId, userId)),
  });
  return !!submission;
}
 ...
 */
export async function getEventFormWithDetails(db: Database, eventId: string, type: "survey" | "evaluation") {
  return await db.query.formEventForm.findFirst({
    where: and(eq(schema.formEventForm.eventId, eventId), eq(schema.formEventForm.type, type)),
    with: {
      form: {
        with: {
          fields: {
 ...
 */
export async function getGroupFormWithDetails(db: Database, groupSlug: string, formId: string) {
  return await db.query.formGroupForm.findFirst({
    where: and(eq(schema.formGroupForm.groupSlug, groupSlug), eq(schema.formGroupForm.formId, formId)),
    with: {
      form: {
        with: {
          fields: {
 ...
 */
export async function createFieldsAndOptions(db: Database, formId: string, fields: CreateFieldInput[]) {
  for (const fieldData of fields) {
    ...
    if (fieldData.options && (fieldData.type === "multiple_select" || fieldData.type === "single_select")) {
 ...
 */
export async function updateFieldsAndOptions(db: Database, formId: string, fields: UpdateFieldInput[]) {
  // Get existing fields
  ...
  const updatedFieldIds = fields.filter((f) => f.id).map((f) => f.id as string);
  ...
  for (const fieldData of fields) {
    if (fieldData.id) {
      ...
      if (fieldData.options && (fieldData.type === "multiple_select" || fieldData.type === "single_select")) {
        ...
        const updatedOptionIds = fieldData.options.filter((o) => o.id).map((o) => o.id as string);
        ...
        await db.delete(schema.formOption).where(eq(schema.formOption.fieldId, fieldData.id));
      }
    } else {
      // Create new field
      const [field] = await db
...
export async function validateAndCreateSubmission(
    ...
    if (eventForm) {
      ...
      if (eventForm.type === "evaluation") {
        const registration = await db.query.eventRegistration.findFirst({
          where: and(
            eq(schema.eventRegistration.eventId, options.eventId),
      ...
      if (existingSubmission && !event?.isRegistrationClosed) {
        await db.delete(schema.formSubmission).where(eq(schema.formSubmission.id, existingSubmission.id));
      }
    }
  }
  ...
  if (options?.groupSlug) {
    const groupForm = await db.query.formGroupForm.findFirst({
      where: and(eq(schema.formGroupForm.formId, formId), eq(schema.formGroupForm.groupSlug, options.groupSlug)),
    });
    ...
    if (groupForm) {
      ...
      if (groupForm.onlyForGroupMembers) {
        const membership = await db.query.groupMembership.findFirst({
          where: and(eq(schema.groupMembership.groupSlug, options.groupSlug), eq(schema.groupMembership.userId, userId)),
        });
 ...
 */
async function createAnswers(db: Database, submissionId: string, answers: CreateAnswerInput[]) {
  for (const answerData of answers) {
    ...
    if (answerData.selected_options && answerData.selected_options.length > 0) {
...
export async function calculateFormStatistics(db: Database, formId: string) {
  ...
  const [submissionCount] = await db.select({ count: count() }).from(schema.formSubmission).where(eq(schema.formSubmission.formId, formId));
  ...
  const statistics = await Promise.all(
    form.fields
      .filter((field) => field.type === "multiple_select" || field.type === "single_select")
      .map(async (field) => {
        const optionsWithStats = await Promise.all(
          field.options.map(async (option) => {
            ...
            const [result] = await db
              ...
              .innerJoin(schema.formAnswer, eq(schema.formAnswerOption.answerId, schema.formAnswer.id))
              .innerJoin(schema.formSubmission, eq(schema.formAnswer.submissionId, schema.formSubmission.id))
              .where(and(eq(schema.formAnswerOption.optionId, option.id), eq(schema.formSubmission.formId, formId)));

            const answerAmount = result?.count || 0;
            const answerPercentage = totalSubmissions > 0 ? Number(((answerAmount / totalSubmissions) * 100).toFixed(2)) : 0;
 ...
 */
export async function canManageForm(db: Database, formId: string, userId: string, hasAdminPermission: boolean): Promise<boolean> {
  ...
  if (eventForm) {
    ...
    if (organizerSlug) {
      const membership = await db.query.groupMembership.findFirst({
        where: and(eq(schema.groupMembership.groupSlug, organizerSlug), eq(schema.groupMembership.userId, userId), eq(schema.groupMembership.role, "leader")),
      });
      return !!membership;
    }
  }
