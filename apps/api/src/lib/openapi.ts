...
import { type DescribeRouteOptions, describeRoute as _describeRoute, resolver, uniqueSymbol } from "hono-openapi";
 ...
 */
const tags = ["api-keys", "assets", "emails", "events", "forms", "groups", "fines", "jobs", "news", "notifications", "payments", "users", "webhooks"] as const;
 ...
 */
type AdditionalDescribeRouteOptions = Omit<DescribeRouteOptions, keyof CustomDescribeRouteOptions>;
...
export class RouteDescriptorBuilder {
   ...
   */
  public response({ statusCode, description, options = {} }: { statusCode: StatusCode; description: string; options?: AdditionalDescribeRouteOptions }) {
  ...
  public badRequest({
    ...
    return this.response({
      statusCode: 400,
      description: description ? `Bad Request - ${description}` : "Bad Request",
      options,
    });
  }
  ...
  public notFound({
    ...
    return this.response({
      statusCode: 404,
      description: description ? `Not Found - ${description}` : "Not Found",
      options,
    });
  }
  ...
  public forbidden({
    ...
    return this.response({
      statusCode: 403,
      description: description ? `Forbidden - ${description}` : "Forbidden",
      options,
    });
  }
  ...
  public unauthorized({
    ...
    return this.response({
      statusCode: 401,
      description: description ? `Unauthorized - ${description}` : "Unauthorized",
      options,
    });
  }
...
export function describeMiddleware<T extends MiddlewareHandler>(middleware: T, spec: DescribeRouteOptions): T {
