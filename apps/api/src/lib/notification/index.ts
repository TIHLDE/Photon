 ...
 */
export async function sendNotification(options: SendNotificationOptions, ctx: AppContext) {
  const { userId, title, description, link, sendTo = { website: true, email: true }, customEmailTemplate } = options;
  ...
  if (shouldSendToEmail && !customEmailTemplate && (!title || !description)) {
    throw new Error("Either customEmailTemplate or both title and description must be provided for email notifications");
  }
