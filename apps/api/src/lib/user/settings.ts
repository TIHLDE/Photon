...
export async function getUserSettings(userId: string, ctx: AppContext): Promise<UserSettings | null> {
  const { db } = ctx;
  ...
  return {
    ...
    receiveMailCommunication: settingsWithAllergies.receiveMailCommunication,
    allergies: settingsWithAllergies.allergies.map((ua) => ua.allergySlug),
  };
}

export async function createUserSettings(userId: string, settings: UserSettings, ctx: AppContext): Promise<UserSettings> {
  const { db } = ctx;
...
export async function updateUserSettings(userId: string, updates: UpdateUserSettings, ctx: AppContext): Promise<UserSettings> {
  const { db } = ctx;
...
export async function setUserAllergies(userId: string, allergySlugs: string[], ctx: AppContext): Promise<void> {
  const { db } = ctx;
