...
const eventMutationSchema = z.object({
  // Core event info
  title: z.string().max(256).meta({ description: "Short title of the event" }),
  description: z.string().meta({ description: "Description of the event" }),
  categorySlug: z.string().meta({ description: "Category slug for the event" }),
  organizerGroupSlug: z.string().meta({ description: "Slug of the group organizing the event" }),
  location: z.string().meta({
    description: "Location of the event (physical or online)",
  }),
  imageUrl: z.url().nullable().meta({ description: "Main image to display for the event" }),
  ...
  priorityPools: z
    .array(
      z.object({
        groups: z.array(z.string()).meta({ description: "Group slugs in this pool" }),
      }),
    )
    .nullable()
    .meta({
...
export const eventDetailSchema = Schema(
  ...
  z.object({
    ...
    location: z.string().nullable().optional().meta({ description: "Event location (nullable)" }),
    startTime: z.iso.datetime().meta({ description: "Event start time (ISO 8601)" }),
    endTime: z.iso.datetime().meta({ description: "Event end time (ISO 8601)" }),
    organizer: z
      .object({
        ...
        image: z.url().nullable().meta({ description: "Organizer image URL" }),
      })
      .nullable()
      .meta({ description: "Event organizer (nullable)" }),
    closed: z.boolean().meta({ description: "Is registration closed" }),
    image: z.url().nullable().meta({ description: "Event image URL (nullable)" }),
    createdAt: z.iso.datetime().meta({ description: "Event creation time (ISO 8601)" }),
    updatedAt: z.iso.datetime().meta({ description: "Event update time (ISO 8601)" }),
    category: z
      .object({
    ...
    payInfo: z
      .object({
        ...
        paymentGracePeriodMinutes: z.number().meta({ description: "Payment grace period in minutes" }),
      })
      .nullable()
    ...
    registration: z
      .object({
        createdAt: z.iso.datetime().meta({ description: "When the user signed up" }),
...
export const eventListItemSchema = Schema(
  ...
  z.object({
    ...
    title: z.string().meta({ description: "Event title" }),
    location: z.string().nullable().optional().meta({ description: "Event location (nullable)" }),
    startTime: z.iso.date().meta({ description: "Event start time (ISO 8601)" }),
    ...
    closed: z.boolean().meta({ description: "Is registration closed" }),
    image: z.url().nullable().meta({ description: "Event image URL (nullable)" }),
    createdAt: z.iso.date().meta({ description: "Event creation time (ISO 8601)" }),
    updatedAt: z.iso.date().meta({ description: "Event update time (ISO 8601)" }),
    category: z
      .object({
        slug: z.string().meta({ description: "Category slug" }),
        label: z.string().meta({ description: "Category label" }),
...
export const registeredUserSchema = Schema(
  ...
  z.object({
    id: z.string().meta({ description: "User id" }),
    name: z.string().meta({ description: "User name" }),
    image: z.string().nullable().meta({ description: "User image url (if any)" }),
  }),
);
...
export const eventRegistrationListResponseSchema = Schema(
  ...
  PagniationResponseSchema.extend({
    registeredUsers: z.array(registeredUserSchema).describe("List of registered users (paginated)"),
  }),
);
