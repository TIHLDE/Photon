...
export const paymentWebhookRoute = route().post(
  ...
  async (c) => {
    ...
    try {
      ...
      let newStatus: "pending" | "paid" | "refunded" | "failed" = "pending";
      ...
      switch (vippsPayment.state) {
        ...
        case "TERMINATED":
          ...
          if (
            ...
            vippsPayment.aggregate.refundedAmount.value === vippsPayment.aggregate.authorizedAmount.value
          ) {
            newStatus = "refunded";
          } else if (vippsPayment.aggregate.capturedAmount.value > 0) {
            newStatus = "paid";
            receivedPaymentAt = new Date();
          } else {
            newStatus = "failed";
      ...
      await db
        ...
        .set({
          status: newStatus,
          receivedPaymentAt: receivedPaymentAt || payment.receivedPaymentAt,
        })
