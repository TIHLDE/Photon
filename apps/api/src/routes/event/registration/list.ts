...
import { PaginationSchema, getPageOffset, getTotalPages } from "~/middleware/pagination";
...
export const getAllRegistrationsForEventsRoute = route().get(
  ...
  async (c) => {
    ...
    const filters = and(eq(schema.eventRegistration.eventId, c.req.param("eventId")), registeredFilter);
