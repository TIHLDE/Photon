  ...
  async (c) => {
    ...
    if (user) {
      const dbRegistration = await db.query.eventRegistration.findFirst({
        where: (registration, { eq }) => eq(registration.userId, user.id),
      });

      if (dbRegistration) {
        registration = {
          attendedAt: dbRegistration.attendedAt?.toISOString() ?? null,
    ...
    const payInfo = event.isPaidEvent
      ? {
          paymentGracePeriodMinutes: event.paymentGracePeriodMinutes || 0,
