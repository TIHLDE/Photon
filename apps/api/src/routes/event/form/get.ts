...
export const getEventFormRoute = route().get(
  ...
  async (c) => {
    ...
    // Get event form
    const eventFormData = await getEventFormWithDetails(db, eventId, formType);
    ...
    if (formType === "evaluation") {
      const hasEventsManage = await hasPermission({ db, ...ctx }, user.id, "events:manage");

      const registration = await db.query.eventRegistration.findFirst({
        where: (reg, { and, eq }) => and(eq(reg.eventId, eventId), eq(reg.userId, user.id)),
      });
      ...
      if (!hasEventsManage && !isAttendee) {
        throw new HTTPException(403, {
          message: "You must have attended the event to access the evaluation",
        });
      }
    }

    const hasAnswered = await userHasSubmitted(db, eventFormData.form.id, user.id);
