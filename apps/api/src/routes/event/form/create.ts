...
export const createEventFormRoute = route().post(
  ...
  async (c) => {
    ...
    const hasEventsWrite = await hasPermission({ db, ...ctx }, user.id, "events:write");

    let isOrganizer = false;
    if (event.organizerGroupSlug) {
      const membership = await db.query.groupMembership.findFirst({
        where: (membership, { and, eq }) =>
          and(eq(membership.groupSlug, event.organizerGroupSlug ?? ""), eq(membership.userId, user.id), eq(membership.role, "leader")),
      });
    ...
    if (!hasEventsWrite && !isOrganizer) {
      throw new HTTPException(403, {
        message: "You do not have permission to create forms for this event",
      });
    }

    // Check if form already exists for this event type
    const existingEventForm = await db.query.formEventForm.findFirst({
      where: (eventForm, { and, eq }) => and(eq(eventForm.eventId, eventId), eq(eventForm.type, body.type)),
    });
