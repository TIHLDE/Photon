...
export const updateRoute = route().put(
  ...
  describeRoute({
    ...
    .forbidden({
      description: "You must be the event creator or have events:update/events:manage permission",
    })
  ...
  async (c) => {
    ...
    await db.transaction(async (tx) => {
      ...
      const existing = await tx.select().from(schema.event).where(eq(schema.event.id, eventId)).limit(1);
      ...
      if (body.categorySlug && body.categorySlug !== event.categorySlug) {
        const category = await tx.select().from(schema.eventCategory).where(eq(schema.eventCategory.slug, body.categorySlug)).limit(1);
      ...
      if (body.organizerGroupSlug && body.organizerGroupSlug !== event.organizerGroupSlug) {
        const group = await tx.select().from(schema.group).where(eq(schema.group.slug, body.organizerGroupSlug)).limit(1);
        if (group.length === 0) {
          throw new HTTPException(400, {
      ...
      if (body.contactPersonUserId && body.contactPersonUserId !== event.contactPersonId) {
        const contactPerson = await tx.select().from(schema.user).where(eq(schema.user.id, body.contactPersonUserId)).limit(1);
      ...
      if (body.title && body.title !== event.title) {
        ...
        if (slug.length > 256) {
          throw new HTTPException(400, {
            message: "Generated slug is too long (> 256 chars). Please use a shorter title",
          });
        }
      }
      ...
      if (body.priorityPools) {
        ...
        await tx.delete(schema.eventPriorityPool).where(eq(schema.eventPriorityPool.eventId, eventId));
      ...
      const updateDateNullable = (date: string | null | undefined): Date | null | undefined => {
      ...
      const updatedEvent: Partial<InferInsertModel<DbSchema["event"]>> = {
        ...
        cancellationDeadline: updateDateNullable(body.cancellationDeadline),
        updateByUserId: userId,
      };

      await tx.update(schema.event).set(updatedEvent).where(eq(schema.event.id, eventId));
    });
