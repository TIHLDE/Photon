...
import { eventListFilterSchema, type eventListItemSchema, eventListResponseSchema } from "./schema";
...
export const listRoute = route().get(
  ...
  async (c) => {
    const { db } = c.get("ctx");
    const { page, pageSize, search, expired, openSignUp } = c.req.valid("query");

    const filters = and(
      ...[
        search ? ilike(schema.event.title, `%${search}%`) : undefined,
        // TODO: Test if works :)
        expired != null ? eq(sql`NOW() > ${schema.event.end}`, expired) : undefined,
        // TODO: Test if works :)
        openSignUp === true
          ? and(eq(schema.event.requiresSigningUp, true), lte(schema.event.registrationEnd, new Date()), gte(schema.event.registrationStart, new Date()))
          : undefined,
      ].filter(Boolean),
    );
