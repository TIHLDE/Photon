  ...
  async (c) => {
    ...
    await db.transaction(async (tx) => {
      ...
      const newEvent: InferInsertModel<DbSchema["event"]> = {
        ...
        cancellationDeadline: body.cancellationDeadline ? new Date(body.cancellationDeadline) : undefined,
      ...
      const [event] = await db.insert(schema.event).values(newEvent).returning({ eventId: schema.event.id });
