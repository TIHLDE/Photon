...
export const deleteRoute = route().delete(
  ...
  describeRoute({
    ...
    .forbidden({
      description: "You must be the event creator or have events:delete permission",
    })
  ...
  async (c) => {
    ...
    return c.json(
      {
        message: c.get("isResourceOwner") ? "Your event has been successfully deleted" : "Event successfully deleted",
      },
      200,
    );
  },
