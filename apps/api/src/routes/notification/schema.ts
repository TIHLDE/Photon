...
export const notificationSchema = Schema(
  ...
  z.object({
    ...
    link: z.string().nullable().meta({ description: "Optional link URL (nullable)" }),
    isRead: z.boolean().meta({ description: "Whether notification is read" }),
    createdAt: z.iso.datetime().meta({ description: "Notification creation time (ISO 8601)" }),
    updatedAt: z.iso.datetime().meta({ description: "Notification update time (ISO 8601)" }),
  }),
);
...
export const deleteNotificationResponseSchema = Schema(
  ...
  z.object({
    success: z.boolean().meta({ description: "Whether deletion was successful" }),
  }),
);
