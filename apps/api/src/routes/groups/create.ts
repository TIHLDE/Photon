...
export const createRoute = route().post(
  ...
  async (c) => {
    ...
    const existingGroup = await db.select().from(schema.group).where(eq(schema.group.slug, body.slug)).limit(1);
    ...
    if (body.finesAdminId) {
      const finesAdmin = await db.select().from(schema.user).where(eq(schema.user.id, body.finesAdminId)).limit(1);
    ...
    const [newGroup] = await db.insert(schema.group).values(body).returning();
