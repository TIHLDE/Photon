...
export const createFineSchema = Schema(
  "CreateFine",
  z.object({
    userId: z.string().max(255).meta({ description: "User ID who receives the fine" }),
    groupSlug: z.string().max(128).meta({ description: "Group slug that issues the fine" }),
    reason: z.string().min(1).meta({ description: "Reason for the fine" }),
    amount: z.number().int().positive().meta({ description: "Fine amount in NOK" }),
...
export const updateFineSchema = Schema(
  ...
  z.object({
    defense: z.string().optional().meta({ description: "User's defense text" }),
    status: z.enum(["pending", "approved", "paid", "rejected"]).optional().meta({ description: "Fine status" }),
    approvedByUserId: z.string().optional().meta({ description: "User who approved the fine" }),
  }),
);
...
export const fineSchema = Schema(
  ...
  z.object({
    ...
    groupSlug: z.string().meta({ description: "Group slug that issued the fine" }),
    ...
    createdByUserId: z.string().nullable().meta({ description: "User who created the fine" }),
    approvedByUserId: z.string().nullable().meta({ description: "User who approved the fine" }),
    approvedAt: z.string().nullable().meta({ description: "Approval timestamp" }),
