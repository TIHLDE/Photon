...
export const listFinesRoute = route().get(
  ...
  async (c) => {
    ...
    const hasScopedPerm = await hasScopedPermission(ctx, user.id, "fines:view", `group:${groupSlug}`);
    ...
    if (statusFilter && (statusFilter === "pending" || statusFilter === "approved" || statusFilter === "paid" || statusFilter === "rejected")) {
