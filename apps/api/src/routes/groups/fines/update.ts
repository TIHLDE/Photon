...
export const updateFineRoute = route().patch(
  ...
  async (c) => {
    ...
    const hasScopedPerm = await hasScopedPermission(ctx, user.id, "fines:update", `group:${groupSlug}`);
    ...
    await db.update(schema.fine).set(updateData).where(eq(schema.fine.id, fineId));
