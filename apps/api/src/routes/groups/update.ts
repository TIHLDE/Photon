...
export const updateRoute = route().patch(
  ...
  describeRoute({
    ...
    .forbidden({
      description: "Not a group leader or missing groups:update permission",
    })
    .notFound({
  ...
  async (c) => {
    ...
    const existingGroup = await db.select().from(schema.group).where(eq(schema.group.slug, slug)).limit(1);
