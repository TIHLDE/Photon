...
export const createGroupFormRoute = route().post(
  ...
  async (c) => {
    ...
    const hasFormsCreate = await hasPermission({ db, ...ctx }, user.id, "forms:create");
    const isGroupLeader = await db.query.groupMembership.findFirst({
      where: (membership, { and, eq }) => and(eq(membership.groupSlug, groupSlug), eq(membership.userId, user.id), eq(membership.role, "leader")),
    });

    if (!hasFormsCreate && !isGroupLeader) {
      throw new HTTPException(403, {
        message: "You do not have permission to create forms for this group",
      });
    }
