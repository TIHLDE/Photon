...
export const updateRoute = route().patch(
  ...
  async (c) => {
    ...
    if (body.classStart && !body.classEnd) {
      ...
      if (classOrder.indexOf(body.classStart) > classOrder.indexOf(job.classEnd)) {
    ...
    if (body.classEnd && !body.classStart) {
      const classOrder = ["first", "second", "third", "fourth", "fifth", "alumni"];
      if (classOrder.indexOf(job.classStart) > classOrder.indexOf(body.classEnd)) {
        throw new HTTPException(400, {
          message: "classStart must be less than or equal to classEnd",
        });
      }
    ...
    const [updatedJob] = await db.update(schema.jobPost).set(updateData).where(eq(schema.jobPost.id, id)).returning();
