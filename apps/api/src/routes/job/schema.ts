...
import { PaginationSchema, PagniationResponseSchema } from "~/middleware/pagination";

// ===== INPUT SCHEMAS =====

export const createJobSchema = z
  .object({
    title: z.string().min(1).max(200).meta({ description: "Job posting title" }),
    ingress: z.string().max(800).default("").meta({ description: "Short description/summary" }),
    body: z.string().default("").meta({ description: "Full job description" }),
    company: z.string().min(1).max(200).meta({ description: "Company name" }),
    location: z.string().min(1).max(200).meta({ description: "Job location" }),
    deadline: z.string().datetime().optional().meta({ description: "Application deadline" }),
    isContinuouslyHiring: z.boolean().default(false).meta({ description: "Whether hiring is ongoing" }),
    jobType: z.enum(["full_time", "part_time", "summer_job", "other"]).default("other").meta({ description: "Type of employment" }),
    email: z.string().email().max(320).optional().meta({ description: "Contact email" }),
    link: z.string().url().optional().meta({ description: "Application or company URL" }),
    classStart: z.enum(["first", "second", "third", "fourth", "fifth", "alumni"]).default("first").meta({ description: "Target class start" }),
    classEnd: z.enum(["first", "second", "third", "fourth", "fifth", "alumni"]).default("fifth").meta({ description: "Target class end" }),
    imageUrl: z.string().url().optional().meta({ description: "Company logo" }),
    imageAlt: z.string().max(255).optional().meta({ description: "Logo alt text" }),
  })
  .refine(
    (data) => {
      const classOrder = ["first", "second", "third", "fourth", "fifth", "alumni"];
      return classOrder.indexOf(data.classStart) <= classOrder.indexOf(data.classEnd);
    },
    {
...
export const updateJobSchema = z
  .object({
    ...
    classStart: z.enum(["first", "second", "third", "fourth", "fifth", "alumni"]).optional(),
    classEnd: z.enum(["first", "second", "third", "fourth", "fifth", "alumni"]).optional(),
...
const jobFields = z.object({
  ...
  deadline: z.string().nullable().meta({ description: "Application deadline (ISO 8601)" }),
