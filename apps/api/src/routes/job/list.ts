...
import { jobListFilterSchema, type jobListItemSchema, jobListResponseSchema } from "./schema";
...
export const listRoute = route().get(
  ...
  async (c) => {
    const { db } = c.get("ctx");
    const { page, pageSize, search, expired, jobType, year } = c.req.valid("query");
    ...
    const conditions = and(
      ...[
        ...
        search ? or(ilike(schema.jobPost.title, `%${search}%`), ilike(schema.jobPost.company, `%${search}%`)) : undefined,
        // Filter by job type
        ...
        year ? and(lte(schema.jobPost.classStart, year), gte(schema.jobPost.classEnd, year)) : undefined,
      ].filter(Boolean),
    );
