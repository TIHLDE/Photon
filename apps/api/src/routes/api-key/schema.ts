...
export const createApiKeySchema = Schema(
  "CreateApiKey",
  z.object({
    name: z.string().min(1).max(100).meta({ description: "Name for the API key" }),
    ...
    metadata: z.record(z.string(), z.unknown()).optional().meta({ description: "Optional metadata as key-value pairs" }),
  }),
);
...
export const updateApiKeySchema = Schema(
  "UpdateApiKey",
  z.object({
    name: z.string().min(1).max(100).optional().meta({ description: "Name for the API key" }),
...
export const apiKeySchema = Schema(
  ...
  z.object({
    ...
    createdById: z.string().nullable().meta({ description: "User ID who created this key" }),
    lastUsedAt: z.iso.datetime().nullable().meta({ description: "When the key was last used" }),
    permissions: z.array(z.string()).meta({
      description: "Array of permissions granted to this API key",
    }),
    metadata: z.record(z.string(), z.unknown()).nullable().meta({ description: "Optional metadata as key-value pairs" }),
    createdAt: z.iso.datetime().meta({ description: "When the key was created" }),
    updatedAt: z.iso.datetime().meta({ description: "When the key was last updated" }),
  }),
);
...
export const listApiKeysResponseSchema = Schema("ApiKeyList", z.array(apiKeySchema));
