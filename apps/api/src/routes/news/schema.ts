...
export const createNewsSchema = z.object({
  title: z.string().min(1).max(200).meta({ description: "News article title" }),
  header: z.string().min(1).max(200).meta({ description: "News article subtitle/ingress" }),
  body: z.string().min(1).meta({ description: "Main content of the news" }),
  imageUrl: z.string().url().optional().meta({ description: "Optional image URL" }),
  imageAlt: z.string().max(255).optional().meta({ description: "Alt text for the image" }),
  emojisAllowed: z.boolean().default(false).meta({ description: "Whether reactions are enabled" }),
});
...
export const createReactionSchema = z.object({
  emoji: z.string().min(1).max(32).meta({ description: "Emoji reaction (e.g., 👍, ❤️, 😂)" }),
});
...
export const newsArticleSchema = Schema(
  ...
  z.object({
    ...
    emojisAllowed: z.boolean().meta({ description: "Whether reactions are enabled" }),
...
export const newsReactionSchema = Schema(
  ...
  z.object({
    ...
    createdAt: z.string().meta({ description: "Reaction creation time (ISO 8601)" }),
  }),
);
