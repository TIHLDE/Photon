...
export const deleteReactionRoute = route().delete(
  ...
  async (c) => {
    ...
    const reaction = await db.query.newsReaction.findFirst({
      where: and(eq(schema.newsReaction.userId, userId), eq(schema.newsReaction.newsId, newsId)),
    });
    ...
    // Delete the reaction
    await db.delete(schema.newsReaction).where(and(eq(schema.newsReaction.userId, userId), eq(schema.newsReaction.newsId, newsId)));
