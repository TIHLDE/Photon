...
export const downloadSubmissionsRoute = route().get(
  ...
  async (c) => {
    ...
    // Check permissions
    const hasAdminPermission = await hasPermission({ db, ...ctx }, user.id, "forms:manage");
    const canManage = await canManageForm(db, formId, user.id, hasAdminPermission);

    if (!canManage) {
      throw new HTTPException(403, {
        message: "You do not have permission to download submissions for this form",
      });
    }
    ...
    if (eventForm) {
      const registrations = await db
        ...
        .where(and(eq(schema.eventRegistration.eventId, eventForm.eventId), eq(schema.eventRegistration.status, "registered")));

      const registeredUserIds = new Set(registrations.map((r) => r.userId));
      submissions = submissions.filter((s) => registeredUserIds.has(s.userId));
    }

    // Build CSV header
    const headers = ["name", "email", "study", "studyyear", ...form.fields.map((field) => field.title)];
    ...
    const rows = submissions.map((submission) => {
      ...
      for (const field of form.fields) {
        ...
        if (answer.answerText) {
          ...
          row.push(answer.selectedOptions.map((so) => so.option.title).join(", "));
        } else {
          row.push("");
        }
      }
