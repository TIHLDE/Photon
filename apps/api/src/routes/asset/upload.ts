import { ALLOWED_MIME_TYPES, MAX_FILE_SIZE, generateAssetKey, isAllowedMimeType } from "~/lib/asset";
...
Send the file as multipart/form-data with a field named "file".`,
  ...
  async (c) => {
    ...
    if (!file || !(file instanceof File)) {
      throw HTTPAppException.BadRequest("No file provided. Send a file with field name 'file'.");
    }

    // Validate file size
    if (file.size > MAX_FILE_SIZE) {
      throw HTTPAppException.BadRequest(`File size exceeds maximum allowed size of ${MAX_FILE_SIZE / 1024 / 1024}MB`);
    }

    // Validate MIME type
    if (!isAllowedMimeType(file.type)) {
      throw HTTPAppException.BadRequest(`File type '${file.type}' is not allowed. Allowed types: ${ALLOWED_MIME_TYPES.join(", ")}`);
    }
    ...
    if (!asset) {
      throw HTTPAppException.InternalError("Failed to create asset record");
    }
