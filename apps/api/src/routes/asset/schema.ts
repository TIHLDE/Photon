...
export const uploadResponseSchema = Schema(
  "UploadResponse",
  z.object({
    key: z.string().meta({ description: "The unique key to reference this asset" }),
    originalFilename: z.string().meta({ description: "The original filename" }),
    contentType: z.string().nullable().meta({ description: "The MIME type of the file" }),
...
export const metadataResponseSchema = Schema(
  ...
  z.object({
    ...
    size: z.number().meta({ description: "File size in bytes" }),
    status: z.enum(["staged", "ready"]).meta({ description: "Asset status" }),
    promotedAt: z.string().datetime().nullable().meta({ description: "When the asset was promoted" }),
    createdAt: z.string().datetime().meta({ description: "When the asset was created" }),
  }),
);
