...
> = FilterKeys<Paths[Path], Method>;

type BodyOf<
    Paths extends object,
    Path extends keyof Paths,
    Method extends HttpMethod,
> = OperationRequestBodyContent<OpOf<Paths, Path, Method>>;
...
> = SuccessResponse<
    Extract<
        ResponseObjectMap<OpOf<Paths, Path, Method>>,
        Record<number | string, unknown>
    >
>;

type PathOf<Paths extends object, Method extends HttpMethod> = PathsWithMethod<
    Paths,
    Method
> &
    string;
...
type QueryParamsOf<Op> = Op extends { parameters: { query?: infer Q } }
    ? NonNullable<Q> extends Record<string, unknown>
        ? NonNullable<Q>
        : never
    : never;
...
type PathParamsOption<Op> = [PathParamsOf<Op>] extends [never]
    ? { params?: never }
    : { params: PathParamsOf<Op> };

type SearchParamsOption<Op> = [QueryParamsOf<Op>] extends [never]
    ? unknown
    : { searchParams?: QueryParamsOf<Op> };

type BodyOption<Body> = [Body] extends [never | undefined]
    ? unknown
    : { json: Body };

type Options<Body = never, Op = unknown> = Omit<
    KyOptions,
    "json" | "searchParams"
> &
    BodyOption<Body> &
    PathParamsOption<Op> &
    SearchParamsOption<Op>;

// Make options required when path params exist
type InitArgs<Body, Op> = [PathParamsOf<Op>] extends [never]
    ? [options?: Options<Body, Op>]
    : [options: Options<Body, Op>];
...
class Client<Paths extends object> {
    ...
    get<Path extends PathOf<Paths, "get">>(
        path: Path,
        ...args: InitArgs<never, OpOf<Paths, Path, "get">>
    ): Promise<SuccessOf<Paths, Path, "get">> {
        return this.request("get", path, args[0]) as Promise<
            SuccessOf<Paths, Path, "get">
        >;
    }
    ...
    post<Path extends PathOf<Paths, "post">>(
        path: Path,
        ...args: InitArgs<
            BodyOf<Paths, Path, "post">,
            OpOf<Paths, Path, "post">
        >
    ): Promise<SuccessOf<Paths, Path, "post">> {
        return this.request("post", path, args[0]) as Promise<
            SuccessOf<Paths, Path, "post">
        >;
    }
    ...
    put<Path extends PathOf<Paths, "put">>(
        ...
        return this.request("put", path, args[0]) as Promise<
            SuccessOf<Paths, Path, "put">
        >;
    }
    ...
    patch<Path extends PathOf<Paths, "patch">>(
        path: Path,
        ...args: InitArgs<
            BodyOf<Paths, Path, "patch">,
            OpOf<Paths, Path, "patch">
        >
    ): Promise<SuccessOf<Paths, Path, "patch">> {
        return this.request("patch", path, args[0]) as Promise<
            SuccessOf<Paths, Path, "patch">
        >;
    }

    delete<Path extends PathOf<Paths, "delete">>(
        path: Path,
        ...args: InitArgs<never, OpOf<Paths, Path, "delete">>
    ): Promise<SuccessOf<Paths, Path, "delete">> {
        return this.request("delete", path, args[0]) as Promise<
            SuccessOf<Paths, Path, "delete">
        >;
    }
...
export function createClient<Paths extends object>(
