...
function TooltipProvider({ delay = DEFAULT_DELAY, ...props }: TooltipPrimitive.Provider.Props) {
...
function TooltipPositioner({ className, ...props }: TooltipPrimitive.Positioner.Props) {
  return (
    <TooltipPrimitive.Portal>
      <TooltipPrimitive.Positioner data-slot="tooltip-positioner" sideOffset={8} className={cn("z-50", className)} {...props} />
    </TooltipPrimitive.Portal>
  );
}
...
function TooltipArrow({ className, ...props }: TooltipPrimitive.Arrow.Props) {
  return (
    <TooltipPrimitive.Arrow
      ...
      className={cn(
        "bg-popover fill-popover z-50 size-2.5 rotate-45 rounded-[2px]",
