...
function Cheatsheet() {
  ...
  useEffect(() => {
    ...
    if (!study || !studyClass || !isURLValid()) {
      navigate(
        linkOptions({
          to: "/kokebok/{-$studyId}/{-$classId}",
          params: { studyId: getUserStudyShort(1), classId: "1" },
          replace: true,
        }),
      );
    }
    ...
    if (newStudy !== getStudy() && newStudy === CheatsheetStudy.DIGTRANS) {
      if (![4, 5].includes(Number(classId))) {
        navigate(
          linkOptions({
            to: "/kokebok/{-$studyId}/{-$classId}",
            params: { studyId: newStudy, classId: "4" },
          }),
        );
      } else {
        navigate(
          linkOptions({
            to: "/kokebok/{-$studyId}/{-$classId}",
            params: { studyId: newStudy, classId: classId },
          }),
        );
      }
      ...
      if (![1, 2, 3].includes(Number(classId))) {
        navigate(
          linkOptions({
            to: "/kokebok/{-$studyId}/{-$classId}",
            params: { studyId: newStudy, classId: "1" },
          }),
        );
      } else {
        navigate(
  ...
  const setClassChoice = (newClass: number) => {
    if (newClass !== getClass()) {
      navigate(
        linkOptions({
          to: "/kokebok/{-$studyId}/{-$classId}",
          params: { studyId: studyId, classId: newClass.toString() },
        }),
      );
    }
  };
